...
参数一：第一个参数是一个类数组对象，即任何可迭代的结构，或者有一个 length 属性 和可索引元素的结构
...
indexOf可以用在查找嵌套数据结构，我常常用在，判断一个数组中对象的索引，然后找到这个对象

在一些情况下，如果能够拿到查找的对象本身，我们就可以通过对象本身+indexOf方法来找到它确切的位置，然后有了索引，数组就很好操作了

示例：

```js
let a = {a: 1}; // 1.可以拿到对象本身
let arr = [a, {b: 2}];
result = arr.indexOf(a); // 2.我们就可以通过a+indexOf方法来找到它在arr数组中的位置
arr.splice(result, 1); //3.然后就可以直接进行操作了(删除)
// 可以利用它来找到对应的对象
```
...
示例：如果找到对应的项是对象类型，可以直接进行修改，原数组中元素也会同步修改（所以我们在实际开发时经常会遇到这样的写法，找到一个对象，然后修改(●'◡'●)）
...
// 就应为result是指向{id: 1, name: 'aa'}这个对象的
result.name = '修改了';

console.log(arr[0].name); // 修改了
```
...
ArrSet(data, 'id'); //根据对象属性id去重
ArrSet(data, 'name'); // 根据对象属性name去重 
```
