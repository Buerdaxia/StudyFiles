...
案例：查询员工表中所有员工的姓名, 和对应的部门名称（左外连接）
...
案例：查询部门表中所有部门的名称, 和对应的员工名称 （右外连接）
...
子查询设计到多张表的查询，终究会导致效率问题，所以我们在原则上，**如果能用连接解决的问题，就不推荐使用子查询**
...
```mysql
-- 1.查询"教研部"部门ID
select id from tb_dept where name = '教研部';    #查询结果：2 只返回了一个值
-- 2.根据"教研部"部门ID, 查询员工信息
...
> 首先先分析表，由于员工表中没有部门名称，只有对应的部门id，所以我们要先吧对应部门名称的对应id找到，再进入到员工表根据找到的id来找对应的人
>
> 分解为以下两步：
>
...
```mysql
-- 1.查询"销售部"和"市场部"的部门ID
select id from tb_dept where name = '教研部' or name = '咨询部';    #查询结果：3,2 返回的数据是一列多行
-- 2.根据部门ID, 查询员工信息
...
简而言之：<b style="color: red">事务是一组操作的集合，它是一个不可分割的工作单位。</b>事务会把所有的操作作为一个整体一起向系统提交或撤销操作请求，即这些操作要么同时成功，要么同时失败。

事务作用：<b style="color: red">保证在一个事务中多次操作数据库表中数据时，要么全都成功,要么全都失败。</b>
...
1. 索引会占用存储空间。
2. **索引大大提高了查询效率，同时却也降低了insert、update、delete的效率。**（原因：除了维护表的数据之外，还要维护索引数据结构，自然而然就会降低效率的）
...
- 为了提高范围查询效率，叶子节点形成了一个双向链表（有序），便于数据的排序及区间范围查询
