...
| is null              | 是null（**不要使用= null**）                 |
| is not null          | 不是null（**不要使用!= null**）              |
...
1. 如果是多字段排序，当第一个字段值相同时，才会根据第二个字段进行排序 以此类推，例如：如果有三个，前两字段值都相同时，才会按照第三个进行排序。
...
> 注意事项:
>
> 1. 起始索引从0开始。        <b style="color: green;">计算公式 ：   起始索引 = （查询页码 - 1）* 每页显示记录数</b>
>
...
~~~mysql
# case流程控制函数
-- case 表达式 when 值1 then 结果1  when 值2  then  结果2 ...  else  兜底结果  end

# 功能和Java中的switch...case一样的
select (case job
             when 1 then '班主任'
             when 2 then '讲师'
             when 3 then '学工主管'
...
从上面两个案例中，我们学习到两个流程函数：

1. `if(表达式, tvalue, fvalue)`: 当表达式为true时，取值为tvalue，当表达式为false时，取fvalue值
2. `case   表达式    when   值1   then  结果1   [when 值2  then  结果2 ...]     [else result]     end`：会根据表达式的值来判断，如果是值1则取结果1以此类推，都不匹配，是else后面的result，**末尾要有一个end表示结束**
...
**一个部门下管理着多个员工，一个员工只归属于一个部门，这种关系就是一对多**
...
- 逻辑外键
  - 概念：在业务层逻辑中，解决外键关联（**通过代码解决一致性完整性问题**）。
