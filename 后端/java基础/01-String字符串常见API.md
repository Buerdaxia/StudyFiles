...
利用compareTo方法可以比较两个字符串的字典值

建议查看JDK的文档里面string方法很多而且描述的很仔细
...
// 诶个对比字典值，如果b>a返回小于0的数，a>b返回一个大于0的数，相等返回0
```
...
1. 有一些`String`类本身没有的方法，例如`reverse`方法
2. 进行字符串拼接时，速度要比单纯使用`String`和`+`速度快的多
...
* **JDK8出现的**（所以用的人比较少，但是很好用）



**构造方法：**

![01-stringJoiner](./../../后端图片/Java基础/01-String/01-stringJoiner.jpg)



**成员方法：**

![02-stringJoiner](./../../后端图片/Java基础/01-String/02-stringJoiner.jpg)
...
结论：如果进行很多字符串变量进行拼接，不要用+号，会严重影响速度，建议使用`StringBuilder`或者`StringJoiner`

`StringBuilder`进行拼接效率高的原因：

**是因为我们从头到位使用的就是一个StringBuilder对象，不会创建新的，所以效率很高**
...
核心：先创建一个StringBuilder对象，然后进行拼接(append方法)，利用String构造函数将StringBuilder变回字符串



所以：

进行一次拼接时（就是有一个+号），就会创建两个对象一个StringBuilder对象，一个String对象
...
核心：会根据字符串，预估一个字符数组，然后将所有字符都放进数组种，再通过String(char [])构造函数直接创建一个字符串
